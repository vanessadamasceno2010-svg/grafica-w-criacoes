...
export function Pedidos() {
    ...
    setSelectedOrder({
      ...
      valor_restante: Number(
        order.valor_restante ||
          Math.max(Number(order.total || 0) - Number(order.valor_entrada || 0), 0)
      ),
  ...
  const openNew = () => {
    setNewOrder(buildInitialPedidoForm());
  };
  ...
  function updateNewOrderMoney(field: 'total' | 'valor_entrada', value: string) {
    ...
    if (total > 0 && next.valor_restante <= 0) {
      next.status_pagamento = 'confirmado';
    } else if (entrada > 0) {
      next.status_pagamento = 'parcial';
    } else {
      next.status_pagamento = 'pendente';
    }
  ...
  const createOrder = async () => {
    ...
    if (!newOrder.cliente_nome.trim()) {
      alert('Informe o nome do cliente.');
      return;
    }

    if (!newOrder.cliente_telefone.trim() && !newOrder.cliente_email.trim()) {
      alert('Informe telefone ou email do cliente.');
      return;
    }

    if (!newOrder.descricao.trim()) {
    ...
    try {
      await apiFetch('/admin/pedidos/manual', {
        ...
        body: JSON.stringify({
          ...
          status_pagamento:
            restante <= 0 ? 'confirmado' : entrada > 0 ? 'parcial' : newOrder.status_pagamento,
    ...
    try {
      await apiFetch('/pedidos/' + selectedOrder.id, {
        ...
        body: JSON.stringify({
          ...
          status_pagamento:
            restante <= 0 ? 'confirmado' : entrada > 0 ? 'parcial' : selectedOrder.status_pagamento,
        ...
        <div style="font-family:Arial;padding:24px;max-width:760px;margin:auto;color:#0b1b3a">
          <div style="display:flex;align-items:center;justify-content:space-between;gap:16px;border-bottom:2px solid #e6aa21;padding-bottom:16px;margin-bottom:20px">
            ...
            <div style="text-align:right">
              <h1 style="margin:0">${tipoTitulo}</h1>
          ...
          <p>
            <b>Total:</b> ${formatMoney(pedido.total)}
            |
            <b>Pago:</b> ${formatMoney(pedido.valor_entrada || 0)}
            |
            <b>Resta:</b> ${formatMoney(pedido.valor_restante || 0)}
          </p>
          <p><b>Prazo de entrega:</b> ${
          ...
          ${
            data.tipo === 'recibo' && data.empresa?.assinatura
              ? `<div style="margin-top:40px;text-align:center"><img src="${data.empresa.assinatura}" style="max-height:90px"/><p>Assinatura digital</p></div>`
              : ''
          }
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Clique em qualquer pedido para abrir, editar e ver histórico.
          </p>
        </div>

        <button className="btn btn-primary w-full sm:w-auto" onClick={openNew}>
          <Plus size={18} />
          Pedido Manual
        </button>
      </div>
        ...
        {filtered.map((o) => {
          ...
          return (
            <button
              key={o.id}
              onClick={() => openOrder(o)}
              className="card p-4 text-left hover:ring-2 hover:ring-gold/40 transition"
            >
              <div className="grid xl:grid-cols-[1fr_1fr_130px_170px_130px] gap-3 items-center">
                <div>
                  <p className="font-bold text-primary">{o.numero_pedido}</p>
                  <p className="text-sm text-gray-500">{statusLabels[o.status] || o.status}</p>
                </div>

                <div>
                  <p className="font-semibold text-primary truncate">
                    {o.cliente_nome || o.cliente_email || 'Cliente'}
                  </p>
                  <p className="text-sm text-gray-500 truncate">{o.cliente_telefone || o.cliente_email}</p>
                </div>

                <div>
                  <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold ${pz.cls}`}>
                    <Icon size={14} />
                    {pz.label}
                  </span>
                  <p className="text-xs text-gray-500 mt-1">
                    {o.prazo_entrega ? new Date(o.prazo_entrega).toLocaleDateString('pt-BR') : 'A combinar'}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    Pago {formatMoney(o.valor_entrada || 0)} • Resta {formatMoney(o.valor_restante || 0)}
                  </p>
                </div>
          ...
          <div className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <input
                ...
                onChange={(e) => setSelectedOrder({ ...selectedOrder, cliente_telefone: e.target.value })}
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <select
                ...
                onChange={(e) => setSelectedOrder({ ...selectedOrder, status_pagamento: e.target.value })}
              >
        ...
        {newOrder && (
          <div className="space-y-4">
            ...
            <div className="sticky bottom-0 bg-white border-t border-gray-100 pt-4 grid grid-cols-2 gap-3">
              <button className="btn btn-outline" onClick={() => setNewOrder(null)}>
                Cancelar
              </button>
