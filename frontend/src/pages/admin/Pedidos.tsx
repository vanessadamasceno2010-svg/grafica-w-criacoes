...
export function Pedidos() {
  ...
  const filtered = useMemo(
    () =>
      orders.filter(
        (o) =>
          String(o.numero_pedido || '').toLowerCase().includes(search.toLowerCase()) ||
          String(o.cliente_nome || o.cliente_email || '').toLowerCase().includes(search.toLowerCase())
      ),
    [orders, search]
  );
  ...
  function selecionarCliente(id: string) {
    ...
    if (!cliente) {
      setNewOrder({ ...newOrder, cliente_id: '', client: '', email: '', phone: '' });
      return;
    }
  ...
  async function createOrder() {
    ...
    try {
      ...
      setNewOrder(null);
      await load();
      alert('Pedido salvo no Supabase.');
    } catch (err: any) {
      alert(err.message);
    }
  }
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map((o) => (
                <tr key={o.id}>
                  ...
                  <td className="px-6 py-4">{statusLabels[o.status] || o.status}</td>
                  <td className="px-6 py-4 font-bold">{formatMoney(o.total)}</td>
                  <td className="px-6 py-4">
                    <div className="flex justify-end gap-2">
                      <button className="p-2 rounded-lg hover:bg-gray-100" onClick={() => setSelectedOrder({ ...o })}>
                        <Eye size={16} />
                      </button>
                      <button className="p-2 rounded-lg hover:bg-gray-100" onClick={() => printReceipt(o)}>
                        <Printer size={16} />
                      </button>
                    </div>
                  </td>
        ...
        <div className="sm:hidden divide-y divide-gray-100">
          {filtered.map((o) => (
            <div key={o.id} className="p-4">
              <h3 className="font-bold text-primary">{o.numero_pedido}</h3>
              <p className="text-sm text-gray-500">{o.cliente_nome || o.cliente_email || 'Cliente'}</p>
              <p className="mt-2">
                <b>{formatMoney(o.total)}</b> • {statusLabels[o.status] || o.status}
              </p>
              <div className="grid grid-cols-2 gap-2 mt-4">
                <button className="btn btn-outline" onClick={() => setSelectedOrder({ ...o })}>
                  <Eye size={16} />
                  Ver
                </button>
                <button className="btn btn-outline" onClick={() => printReceipt(o)}>
                  <Printer size={16} />
                  Recibo
                </button>
              </div>
      ...
      <BottomSheet isOpen={!!selectedOrder} onClose={() => setSelectedOrder(null)} title="Pedido">
        {selectedOrder && (
          <div className="space-y-4">
            ...
            <select className="input" value={selectedOrder.status} onChange={(e) => setSelectedOrder({ ...selectedOrder, status: e.target.value })}>
              {Object.entries(statusLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}
            </select>
            <select className="input" value={selectedOrder.status_pagamento || 'pendente'} onChange={(e) => setSelectedOrder({ ...selectedOrder, status_pagamento: e.target.value })}>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button className="btn btn-outline" onClick={() => printReceipt(selectedOrder)}>Recibo</button>
        ...
        {newOrder && (
          <div className="space-y-4">
            <label className="block">
              <span className="text-sm font-bold text-gray-700">Selecionar cliente cadastrado</span>
              <select className="input mt-1" value={newOrder.cliente_id} onChange={(e) => selecionarCliente(e.target.value)}>
                <option value="">Inserir cliente manualmente</option>
            ...
            <input className="input" placeholder="Total R$" value={newOrder.total} onChange={(e) => setNewOrder({ ...newOrder, total: e.target.value })} />
