...
export function PDV() {
  ...
  return (
    <div className="fade-in p-4 sm:p-6 lg:p-8">
      ...
      <details className="card p-4 mb-4"><summary className="cursor-pointer font-semibold">Impressão térmica direta — Elgin L42 Pro</summary><div className="flex flex-wrap gap-4 mt-3"><label className="text-sm">Largura da etiqueta (mm)<input type="number" min="40" max="108" className="input mt-1" value={labelWidth} onChange={e=>{setLabelWidth(e.target.value);localStorage.setItem('elgin_label_width',e.target.value);}}/></label><label className="text-sm">Altura da etiqueta (mm)<input type="number" min="40" max="300" className="input mt-1" value={labelHeight} onChange={e=>{setLabelHeight(e.target.value);localStorage.setItem('elgin_label_height',e.target.value);}}/></label></div><p className="text-sm text-gray-500 mt-3">Use etiqueta termossensível sem ribbon. Nas preferências da impressora, selecione <b>Termo direto</b>, não “transferência térmica”. Escala 100%, sem cabeçalhos e rodapés.</p></details>
