...
export function ProductCard({ product }: ProductCardProps) {
  ...
  const handleShare = async () => {
    try {
      await shareProduct(product, { price });
    } catch (error) {
      console.error(error);
  ...
  return (
    <article className="group card flex flex-col h-full active:scale-[0.98] transition-transform duration-200 overflow-hidden">
      <Link to={`/produto/${target}`} className="flex flex-col flex-1 cursor-pointer">
      ...
      <div className="px-4 pb-4 -mt-1">
        <button
          type="button"
          onClick={handleShare}
          className="w-full h-11 rounded-2xl border border-gray-200 bg-white text-primary font-bold text-sm flex items-center justify-center gap-2 active:scale-[0.98] hover:bg-gray-50 transition-all"
