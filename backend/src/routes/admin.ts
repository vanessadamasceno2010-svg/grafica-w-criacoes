...
function isStaff(user: any) {
  ...
  return role === 'admin' || role === 'funcionario' || role === 'staff' || role === 'employee';
}
...
adminRoutes.get('/dashboard', asyncHandler(async (_req, res) => {
  ...
  const now = new Date();
  const pedidosMes = pedidos.filter((p) => { const d = new Date(p.created_at); return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear(); });
  ...
  res.json({
    ...
    clientesNovos: users.filter((u) => { const d = new Date(u.created_at); return u.role === 'user' && d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear(); }).length,
...
adminRoutes.get('/clientes', asyncHandler(async (_req, res) => {
  ...
  res.json(users.map((u) => {
    ...
    return { ...u, total_gasto: userOrders.reduce((sum, p) => sum + Number(p.total || 0), 0), pedidos: userOrders.length };
  }));
}));
...
adminRoutes.post('/clientes', admin, asyncHandler(async (req, res) => {
  ...
  const rows = await supabaseRest<any[]>('/users', { method: 'POST', body: JSON.stringify({ nome: d.nome, email: d.email.toLowerCase(), telefone: d.telefone, senha: senhaHash, role: 'user', created_at: new Date().toISOString(), updated_at: new Date().toISOString() }) });
...
adminRoutes.put('/configuracoes/:chave', admin, asyncHandler(async (req, res) => {
  const d = z.object({ valor: z.string(), tipo: z.string().optional().default('texto') }).parse(req.body);
...
adminRoutes.put('/usuarios/:id', admin, asyncHandler(async (req, res) => {
  const d = z.object({ nome: z.string().optional(), telefone: z.string().optional(), role: z.enum(['user', 'admin', 'funcionario', 'inactive']).optional() }).parse(req.body);
  const rows = await supabaseRest<any[]>(`/users?id=eq.${restEq(req.params.id)}`, { method: 'PATCH', body: JSON.stringify({ ...d, updated_at: new Date().toISOString() }) });
  res.json(rows[0] || { ok: true });
}));
