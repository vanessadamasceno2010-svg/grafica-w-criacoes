...
function normalizeProduct(product: any, categories: any[] = []) {
  ...
  return {
    ...
    variacoes: Array.isArray(product.variacoes) ? product.variacoes.map((v: any, index: number) => ({
      id: v?.id || String(index + 1),
      nome: v?.nome || `Variação ${index + 1}`,
      quantidade: v?.quantidade || '',
      preco: Number(v?.preco || 0),
      preco_original: v?.preco_original !== undefined && v?.preco_original !== null ? Number(v.preco_original) : null,
